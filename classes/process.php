<?php
    class Process{
        ...
        public function get_type($MYSQL){
                $sql ="SELECT `type`.`typeid`,`type`.`typename` FROM `scholar`.`type`";   
    
                $listType = $MYSQL->getResults($sql);
    	...
    	public function save_param_details($MYSQL){
    				require "lang/en.php";
    			if((isset($_POST["save_details"])) OR (isset($_POST["update_details"]))){
    				...
    				if(isset($_POST["update_details"])){
                        ...
                        $param_enddate = ["endate"=>$enddate];
                        $update_param_data = array_merge($param_data,$param_enddate);
    					$update_param = $MYSQL->update($_SESSION["table_name"], $update_param_data , $where);
    					if($update_user == TRUE){
    						if(isset($_SESSION["editId"])){ unset($_SESSION["editId"]); }
    						header("Location: ./"); 
    						print $lang["ins_success"]; 
